...
func (dc *dynCompiler) checkPrimitiveSchema(dyn *model.DynValue, schema *model.OpenAPISchema) {
	...
	dyn.Value = dc.convertToType(dyn.Value, schema)

	// Check whether the input value is one of the enumerated types.
	if len(schema.Enum) > 0 {
		for _, e := range schema.Enum {
			val := dc.convertToType(e, schema)
...
func (dc *dynCompiler) checkMapSchema(dyn *model.DynValue, schema *model.OpenAPISchema) {
	...
	for prop, propSchema := range schema.Properties {
		...
		field.Ref.Value = dc.convertToType(propSchema.DefaultValue, propSchema)
...
func (dc *dynCompiler) convertToType(val interface{}, schema *model.OpenAPISchema) model.ValueNode {
	...
	if !isValueNode {
		...
		case []interface{}:
			vn = model.NewListValue()
		case map[string]interface{}:
			vn = model.NewMapValue()
		default:
			dc.reportError(common.NoLocation,
